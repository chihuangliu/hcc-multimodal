...
def extract_embeddings(
    ...
    cols = [f"img_emb_{i}" for i in range(embed_dim)] + [f"gene_emb_{i}" for i in range(embed_dim)]
    rows = []
    for pid in valid:
...
def load_radiomics(patient_ids: set[int]) -> pd.DataFrame:
    ...
    df["patient_id"] = df["Scan name"].str.replace(r"\.nii\.gz$", "", regex=True).astype(int)
...
def load_radiomics_baseline(outcome: str) -> tuple[pd.DataFrame, pd.Series, dict]:
    ...
    clinical = add_rfs_columns(
        pd.read_csv(_CLINICAL_PATH).dropna(how="all")
    )
    arterial = pd.read_csv(_RADIOMICS_PATH).dropna(how="all")
    arterial["SID"] = arterial["Scan name"].str.replace(r"\.nii\.gz$", "", regex=True).astype(int)
...
def _cv(
    ...
    if selector_before_cv:
        ...
        X, y, xcols, cv_kwargs = apply_selector_before_cv(
            X, y, xcols, sel, selector_first=False, label=label, save_path=save_path,
        )
        return run_cv_experiment(
            X, y, xcols, label=label, n_splits=n_splits,
            models=models, return_proba=return_proba, **cv_kwargs,
        )

    return run_cv_experiment(
        X, y, xcols, label=label, n_splits=n_splits,
        models=models, feature_selector=sel, selector_first=False,
...
def _run_ensemble(
    ...
    _, emb_folds = _cv(
        emb_common, y_common, f"emb_{outcome}", k, n_splits,
        selector_before_cv, output_dir, return_proba=True,
    )
    _, radio_folds = _cv(
        radio_common, y_common, f"radio_{outcome}", k, n_splits,
        selector_before_cv, output_dir, return_proba=True,
    )
...
_TRAINING_ROOT = Path(hcc_multimodal.__file__).parent.parent / "training" / "contrastive"
_RESULTS_ROOT = Path(hcc_multimodal.__file__).parent.parent / "results" / "multimodal_prediction"
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.task == "radiomics":
        ...
        summary_df, fold_df = _cv(
            X_r, y_r, f"radiomics_{args.outcome}", args.k, args.n_splits,
            args.selector_before_cv, output_dir, models=radio_models,
        )
        ...
        emb_df = extract_embeddings(
            img_enc, gene_enc, gene_matrix, outcomes, meta,
        ...
        if args.task == "embeddings":
            summary_df, fold_df = _cv(
                emb_df, y, f"contrastive_emb_{args.outcome}", args.k, args.n_splits,
                args.selector_before_cv, output_dir,
            )
            fold_out = fold_df[["experiment", "model", "fold", "train_auc", "test_auc"]]
            ...
            print(f"Combined: emb {emb_df.shape[1]} + radiomics {radio_df.shape[1]} = {combined.shape[1]} features")
            summary_df, fold_df = _cv(
                combined, y.loc[common], f"concat_{args.outcome}", args.k, args.n_splits,
                args.selector_before_cv, output_dir,
            )
            fold_out = fold_df[["experiment", "model", "fold", "train_auc", "test_auc"]]
            ...
            summary_df, fold_df = _run_ensemble(
                emb_df, radio_df, y, args.outcome, args.k, args.n_splits,
                args.selector_before_cv, output_dir,
            )
            fold_out = fold_df[["experiment", "model", "fold", "emb_auc", "radio_auc", "ensemble_auc"]]
    ...
    print("\n=== Results ===")
    print(summary_df[["experiment", "model", "AUC mean", "AUC std"]].to_string(index=False))
    print(f"\nSaved → {output_dir}/")
...
def _parse() -> argparse.Namespace:
    ...
    p.add_argument(
        "--outcome", default="rfs_2year", choices=["rfs_1year", "rfs_2year"],
    ...
    p.add_argument(
        "--selector_before_cv", action="store_true",
        help="Apply SelectKBest before CV (label-leaking, diagnostic only)",
    )
    p.add_argument("--batch_size", type=int, default=32, help="Batch size for embedding extraction")
