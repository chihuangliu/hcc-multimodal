...
| 4 | **Feature selection helps survival, not AUROC.** LASSO/Boruta gives the best Soramic split (C-index 0.578, HR 1.78 [0.91–3.49], log-rank **p=0.088** under kmeans-log / 0.103 median, balanced 71/29) vs the degenerate 98–99/1 splits Ridge/EN + Variance produce under the frozen cutoff. Under the in-sample-frozen cutoff no head reaches <0.05. |
...
The frozen threshold is the median (and k-means / Youden boundary) of the **in-sample refit
resection scores** (`--freeze-on insample`) — the median of the actual score vector each head
produces on resection, not the out-of-fold scores used in earlier drafts. AUROC/C-index are
cutoff-free; `—` = imbalanced (<5 in one arm).

### 7.1 Soramic — top-5 heads × cutoff

| Head | Cutoff | AUROC | C-idx | n hi/lo | HR (95% CI) | log-rank p | Median RFS hi/lo |
|---|---|---:|---:|---|---|---:|---|
| LASSO / All | median / youden | 0.736 | 0.529 | 85/15 | 1.68 (0.75–3.76) | 0.205 | 15.8 / 34.0 |
| | kmeans | | | 86/14 | 1.77 (0.75–4.19) | 0.188 | |
| | kmeans-log | | | 91/9 | 1.53 (0.47–4.96) | 0.472 | |
| LASSO / LASSO | median / kmeans / youden | 0.731 | 0.525 | 93/7 | 1.72 (0.42–7.13) | 0.448 | 16.4 / 34.0 |
| Ridge / Variance | (all) | 0.726 | 0.541 | 98–99/1–2 | — | — | — |
| **LASSO / Boruta** | **kmeans-log** | **0.724** | **0.578** | **71/29** | **1.78 (0.91–3.49)** | **0.088** | **15.2 / 29.0** |
| | median / kmeans / youden | | | 66/34 | 1.67 (0.90–3.11) | 0.103 | 16.4 / 29.0 |
| Elastic Net / Variance | (all) | 0.721 | 0.542 | 98–99/1 | — | — | — |

**LASSO/Boruta** is still the strongest balanced split (highest C-index; lowest log-rank p — 0.088
under kmeans-log, 0.103 under median), but under the in-sample-frozen cutoff **no head reaches
p<0.05 on Soramic**. Raising the threshold to the in-sample median shifts a few Soramic patients
across the boundary and dilutes the separation (e.g. LASSO/All median p 0.096→0.205 vs the
OOF-frozen cutoff). Ridge/EN + Variance still collapse to 98–99/1 because their Soramic scores sit
almost entirely above the boundary; Boruta's compact subset spreads scores enough for a balanced split.

### 7.2 Best head applied to Lausanne

Best Soramic head = **LASSO/Boruta + kmeans_log_frozen** (lowest log-rank p among balanced,
correct-direction splits), applied to Lausanne:

| Cohort | Cutoff | AUROC | C-idx | n hi/lo | HR (95% CI) | log-rank p | Median RFS hi/lo |
|---|---|---:|---:|---|---|---:|---|
| Soramic | kmeans-log | 0.724 | 0.578 | 71/29 | 1.78 (0.91–3.49) | 0.088 | 15.2 / 29.0 |
| Lausanne | kmeans-log | 0.487 | 0.558 | 37/31 | 1.02 (0.62–1.68) | 0.936 | 9.6 / 11.9 |
| Lausanne | median / kmeans / youden | | | 34/34, 36/32 | 1.23–1.30 | 0.31–0.42 | |

![KM — best grid head, Soramic vs Lausanne](km_grid_best.png)
*LASSO/Boruta + kmeans_log_frozen. Soramic separates (p=0.088); Lausanne overlaps (p=0.936). SVG: `km_grid_best.svg`.*
...
4. **FS matters more for survival** — Boruta barely changes AUROC (0.724 vs 0.736) but is decisive for a balanced split (lowest log-rank p, 0.088) where the linear+variance heads collapse to 98–99/1.
